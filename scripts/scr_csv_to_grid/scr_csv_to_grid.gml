...
function scr_csv_to_grid() {
  ...
  //Handle arguments
  if (argument_count < 1 || argument_count > 5) {
    show_error(
      "Incorrect number of arguments (" + string(argument_count) + ")",
      false
    );
    return undefined;
  }
  ...
  if (argument_count >= 2 && !is_undefined(argument[1]))
    _force_strings = argument[1];
  if (argument_count >= 3 && !is_undefined(argument[2]))
    _cell_delimiter = argument[2];
  if (argument_count >= 4 && !is_undefined(argument[3]))
    _string_delimiter = argument[3];
  if (argument_count >= 5 && !is_undefined(argument[4]))
    _newline_alt = argument[4];

  //Check for silliness...
  if (
    string_length(_cell_delimiter) != 1 ||
    string_length(_string_delimiter) != 1
  ) {
  ...
  if (!(_bom_a == 239 && _bom_b == 187 && _bom_c == 191)) {
    show_debug_message(
      "CAUTION: csv_to_grid: " +
        _filename +
        ": CSV file might not be UTF-8 encoded (no BOM)"
    );
    buffer_seek(_buffer, buffer_seek_start, 0);
  } else {
    _size -= 3;
  ...
  for (var _i = 0; _i < _size; _i++) {
    ...
    if (_newline_alt) {
      ...
      var _newline = _prev_val == 13 && _val == 10;

      //If we've found a newline and we're in a string, skip over the chr(10) character
      if (_in_string && _newline) continue;
    }

    //If we've found a new cell
    if ((_val == _cell_delimiter_ord || _newline) && !_in_string) {
      _sheet_width++;
      ...
      ds_grid_set(_grid, _sheet_width - 1, _sheet_height - 1, _str);
