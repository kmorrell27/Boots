...
function StateMachine(_state) constructor {
  ...
  if (variable_instance_exists(other, "__stateStruct__")) {
    var _message =
      "State Machine already exists for " +
      string(object_get_name(other.object_index)) +
      ".";
  ...
  static add = function(_name, _struct) {
    ...
    if (!variable_struct_exists(_struct, "enter"))
      _struct.enter = function() {};
    if (!variable_struct_exists(_struct, "step")) _struct.step = function() {};
    if (!variable_struct_exists(_struct, "draw"))
      _struct.draw = function() {
        draw_self();
      };
    if (!variable_struct_exists(_struct, "leave"))
      _struct.leave = function() {};

    variable_struct_set(states, _name, [
      method(other, _struct.enter),
