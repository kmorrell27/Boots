...
if (place_meeting(x, y, objLink) && !objLink.jumping) {
  global.lastlinkx = objLink.x * (display_get_gui_width() / camera_get_view_width(view_camera[0]));
  //Store Link's X coordinate.
  global.lastlinky = objLink.y * (display_get_gui_height() / camera_get_view_height(view_camera[0]));
